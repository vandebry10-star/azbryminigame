// === AZBRY CHESS MAIN.JS (CLICKABLE, SYNC DGN ENGINE) ===
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (!engine) { console.error('Engine tidak ditemukan'); return; }

  let mode='human', sel=null, last=null, flipped=false;

  // build grid 8x8 + label
  function buildGrid(){
    boardEl.innerHTML = '';
  ...
  function buildLabels(){
    ...
    for (let r=8;r>=1;r--){
      const s=document.createElement('span'); s.className='rank'; s.textContent=r; s.style.top=`${(8-r)*12.5}%`; layer.appendChild(s);
    }
    for (let i=0;i<8;i++){
      const s=document.createElement('span'); s.className='file'; s.textContent=FILES[i]; s.style.left=`${i*12.5}%`; layer.appendChild(s);
    }
  ...
  function renderFromFEN(fen){
    ...
    for (let r=0;r<8;r++){
      ...
      for (const ch of rows[r]){
        ...
        const sq=FILES[file]+(8-r), cell=findCell(sq);
        if (cell){
  ...
  function hasColorPiece(sq,color){ const p=engine.get(sq); return p && p.color===color; }
  function findKingSq(color){
    const fen=engine.fen().split(' ')[0]; const rows=fen.split('/');
    for (let r=0;r<8;r++){ let f=0; for (const ch of rows[r]){
      if (/\d/.test(ch)){ f+=parseInt(ch,10); continue; }
      const sq=FILES[f]+(8-r); if (color==='w'&&ch==='K') return sq; if (color==='b'&&ch==='k') return sq; f++;
    }}
    return null;
  }
  function legalTargetsFrom(square){
    return engine.moves({ square }).map(m=>m.to);
  }

  function renderEngine(){
    renderFromFEN(engine.fen());
    if (last){ findCell(last.from)?.classList.add('src'); findCell(last.to)?.classList.add('dst'); }
    // (engine mini tidak ada in_check yang real)
    if (moveLog){
  ...
  function aiMove(){
    const legal=engine.moves(); if (!legal.length) return;
    const mv=legal[Math.floor(Math.random()*legal.length)];
    engine.move(mv); last={from:mv.from,to:mv.to}; sync();
  }
  function sync(){ renderEngine(); }

  // delegated click
  boardEl.addEventListener('click', (e)=>{
    const sqEl=e.target.closest('.square'); if(!sqEl||!boardEl.contains(sqEl)) return;
    ...
    if (sel){
      ...
      if (legal.includes(sq)){
        ...
        if (mv){ last={from:sel,to:sq}; sel=null; sync(); if (mode==='ai') setTimeout(aiMove,250); return; }
      }
    }
  ...
  btnAI    && (btnAI.onclick   =()=>{ mode='ai';    sel=null; last=null; btnAI.classList.add('active');    btnHuman?.classList.remove('active'); sync(); if (engine.turn()==='b') setTimeout(aiMove,250); });
  btnReset && (btnReset.onclick=()=>{ engine.reset(); sel=null; last=null; sync(); });
  btnUndo  && (btnUndo.onclick =()=>{ engine.undo&&engine.undo(); sel=null; last=null; sync(); });
  btnRedo  && (btnRedo.onclick =()=>{ engine.redo&&engine.redo(); sync(); });
